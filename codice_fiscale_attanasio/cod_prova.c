...
void cognome()                          //funzione che compone il cognome nel codice fiscale
{
    ...
    if(cons >= 3){
        ...
        for(i = 0 ; i < cgl ; i++)
            codice_fiscale[i] = con[i];
    }else if(cons == 1 && vocs >= 2){  //info: sul sito dell'agenzia delle entrate c'è solo uguale a '2'
        con[1] = voc[0];
        con[2] = voc[1];
        for(i = 0 ; i < cgl ; i++)
            codice_fiscale[i] = con[i];
...
void nome(){                            //funzione che compone il nome nel codice fiscale
    char nome[len];
...
void anno(){                                //funzione che compone le due cifre relative all'anno nel codice fiscale
    int anno_nascita;
...
char codici_catastali(){
    ...
    printf("Inserisci il comune di dove sei originario: ");
    scanf("%s", comune);
...
void inverso(){
    ...
    printf("IL COMUNE DI ORIGINE: %s", riga);
    fclose(fp);
}
